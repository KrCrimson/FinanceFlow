...
export default function ReportsScreen({ isDarkMode, currency }) {
  ...
  const categoriesList = Object.keys(categoryStats)
    .map((cat) => ({
      name: cat,
  ...
  const allCategories = [
    "Todas las categorías",
    ...Array.from(new Set(activos.map((m) => m.categoria))),
  ];
      ...
      <View style={theme.tabHeader}>
        ...
        >
          <Text
            style={[
              theme.tabBtnText,
              activeTab === "resumen" && theme.tabBtnTextActive,
            ]}
          >
            📈 Resumen General
          </Text>
        ...
        >
          <Text
            style={[
              theme.tabBtnText,
              activeTab === "graficos" && theme.tabBtnTextActive,
            ]}
          >
            📊 Gráficos
          </Text>
        ...
        <TouchableOpacity
          style={[
            theme.tabBtn,
            activeTab === "movimientos" && theme.tabBtnActive,
          ]}
        ...
        >
          <Text
            style={[
              theme.tabBtnText,
              activeTab === "movimientos" && theme.tabBtnTextActive,
            ]}
          >
            💰 Movimientos
          </Text>
            ...
            <View style={theme.topCardsGrid}>
              ...
              >
                <View style={theme.topCardRow}>
                  <Text style={[theme.topCardLabel, { color: "#10B981" }]}>
                    Total Ingresos
                  </Text>
                  <Text style={theme.topCardIcon}>📈</Text>
                </View>
                <Text
                  style={[theme.topCardValue, { color: "#10B981" }]}
                  numberOfLines={1}
                >
              ...
              >
                <View style={theme.topCardRow}>
                  <Text style={[theme.topCardLabel, { color: "#EF4444" }]}>
                    Total Egresos
                  </Text>
                  <Text style={theme.topCardIcon}>📉</Text>
                </View>
                <Text
                  style={[theme.topCardValue, { color: "#EF4444" }]}
                  numberOfLines={1}
                >
                  {currency || getMobileCurrencySymbol()}{" "}
              ...
              >
                <View style={theme.topCardRow}>
                  <Text style={[theme.topCardLabel, { color: "#60A5FA" }]}>
                    Balance
                  </Text>
                  <Text style={theme.topCardIcon}>💰</Text>
                </View>
                <Text
                  style={[theme.topCardValue, { color: "#60A5FA" }]}
                  numberOfLines={1}
                >
                  {currency || getMobileCurrencySymbol()}{" "}
                  ...
                  {categoriesList.map((cat) => (
                    <View key={cat.name} style={theme.catCard}>
                      <View style={theme.catCardHeader}>
                        <Text style={theme.catCardName} numberOfLines={1}>
                          {cat.name}
                        </Text>
                        <Text style={theme.catCardCount}>({cat.count})</Text>
                      </View>
                      <Text style={theme.catCardAmount}>
                        {currency || getMobileCurrencySymbol()}{" "}
        ...
        {activeTab === "graficos" && (
          <View>
            ...
            <View style={theme.card}>
              ...
              <Text style={theme.cardSub}>
                Monto Total Egresos: {currency || getMobileCurrencySymbol()}{" "}
                {totalEgresos.toFixed(2)}
              </Text>
              ...
              <View style={theme.multiSegmentTrack}>
                {egresosCategories.map((cat, idx) => {
                  const percent =
                    totalEgresos > 0 ? (cat.amount / totalEgresos) * 100 : 0;
                  const color = CATEGORY_COLORS[idx % CATEGORY_COLORS.length];
                  return (
                    <View
                      key={cat.name}
                      style={[
                        theme.multiSegmentFill,
              ...
              <View style={theme.legendGrid}>
                {egresosCategories.map((cat, idx) => {
                  ...
                  const percent =
                    totalEgresos > 0
                      ? ((cat.amount / totalEgresos) * 100).toFixed(1)
                      : "0";
                  return (
                    <View key={cat.name} style={theme.legendBadge}>
                      <View
                        style={[theme.legendDot, { backgroundColor: color }]}
                      />
                      <Text style={theme.legendText}>
                        {cat.name} ({percent}%)
                      </Text>
                    </View>
                  );
              ...
              <View style={theme.barChartContainer}>
                {ingresosCategories.length === 0 ? (
                  ...
                  ingresosCategories.map((cat, idx) => {
                    const color =
                      CATEGORY_COLORS[(idx + 1) % CATEGORY_COLORS.length];
                    const maxIngreso = Math.max(
                      ...ingresosCategories.map((c) => c.amount),
                      1,
                    );
                    const barHeightPercent = Math.min(
                      100,
                      Math.max(15, (cat.amount / maxIngreso) * 100),
                    );

                    return (
                      <View key={cat.name} style={theme.barGroup}>
                        <Text style={theme.barLabel}>
                        ...
                        <Text style={theme.barCatName} numberOfLines={1}>
                          {cat.name}
                        </Text>
                      </View>
                    );
            ...
            <View style={theme.card}>
              ...
              <View style={theme.monthChartContainer}>
                {trendMonths.map((m) => {
                  const barPercent = Math.min(
                    100,
                    Math.max(10, (m.ingreso / 3000) * 100),
                  );
                  return (
                    <View key={m.label} style={theme.monthBarGroup}>
            ...
            <View style={theme.card}>
              ...
              <View style={theme.statsRowGrid}>
                <View style={theme.statMetricCard}>
                  <Text style={theme.statMetricLabel}>
                    💸 Gasto Promedio Mensual
                  </Text>
                  <Text style={[theme.statMetricValue, { color: "#EF4444" }]}>
                    S/ {(totalEgresos / 6).toFixed(2)}
                  </Text>
                </View>
                <View style={theme.statMetricCard}>
                  <Text style={theme.statMetricLabel}>
                  ...
                  <Text style={[theme.statMetricValue, { color: "#10B981" }]}>
                    S/ {(totalIngresos / 6).toFixed(2)}
                  </Text>
                </View>
                <View style={theme.statMetricCard}>
                  <Text style={theme.statMetricLabel}>⚖️ Balance Promedio</Text>
                  <Text style={[theme.statMetricValue, { color: "#3B82F6" }]}>
                    S/ {(balanceNeto / 6).toFixed(2)}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View>
            ...
            <View style={theme.filterCard}>
              <View style={theme.filterRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <TouchableOpacity
                    style={theme.filterDropdownBtn}
                    onPress={() => setShowMonthModal(true)}
                  >
                    <Text style={theme.filterDropdownText} numberOfLines={1}>
                      {selectedMonth} ∨
                    </Text>
                  </TouchableOpacity>
                </View>
            ...
            <View style={theme.card}>
              <View style={theme.tableHeaderRow}>
                <Text style={[theme.tableColHeader, { flex: 1 }]}>
                  Tipo / Nombre
                </Text>
                <Text
                  style={[
                    theme.tableColHeader,
                  ...
                  <View key={item._id} style={theme.tableRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        >
                          <Text style={theme.badgeTypeText}>
                            {item.tipo === "ingreso"
                              ? "💰 ingreso"
                              : "💸 egreso"}
                          </Text>
                      ...
                      <Text style={theme.tableItemName} numberOfLines={1}>
                        {item.nombre}
                      </Text>
                    </View>
                    ...
                    <Text
                      style={[
                        theme.tableItemAmount,
                        {
                          flex: 0.8,
                          textAlign: "right",
                          color:
                            item.tipo === "ingreso" ? "#10B981" : "#EF4444",
                        },
                      ]}
                      numberOfLines={1}
                    >
