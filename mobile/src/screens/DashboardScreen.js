...
export default function DashboardScreen({
  ...
  const loadData = async () => {
    try {
      ...
      if (dataPendientes?.yesterday && !dataPendientes.yesterday.isClosed) {
        triggerNotification(
          "📅 Arqueo Diario Pendiente",
          `Tienes un arqueo de caja pendiente para el día ${dataPendientes.yesterday.date}.`,
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleConfirmarCierre = async () => {
    ...
    try {
      ...
      Alert.alert(
        "¡Cierre Exitoso!",
        `El arqueo ${cierreTipo} ha sido registrado de forma segura.`,
      );
      await loadData();
    } catch (err) {
      setCierreError(
        err.message || "Error al guardar el cierre. Verifique sus datos.",
      );
    } finally {
  ...
  const handleAddCompra = async () => {
    ...
    try {
      ...
      Alert.alert(
        "¡Meta Agregada!",
        "Tu meta de compra ha sido añadida y sincronizada.",
      );
    } catch (err) {
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "🎉 ¡Felicidades!",
                `Se registró el egreso de S/ ${compra.montoObjetivo} en tus movimientos.`,
              );
            } catch (err) {
        ...
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Meta Cancelada",
                "La planificación ha sido removida.",
              );
            } catch (err) {
  ...
  // Botón "Cancelar Constancia"
  const handleCancelarConstancia = (id, nombre) => {
    Alert.alert(
      "🔄 Cancelar Constancia",
      `¿Deseas cancelar la recurrencia de "${nombre}"?\nNo se autogenerará en los siguientes meses.`,
      [
        { text: "No", style: "cancel" },
        {
          text: "Sí, Cancelar",
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                ...
                "El ingreso ya no es recurrente.",
              );
            } catch (err) {
              Alert.alert("Error", "No se pudo cancelar la constancia.");
            }
          },
        },
      ],
  ...
  // Alertas de consumo similares a la web
  const alertas = [];
  if (totalIngresos > 0) {
    ...
    filteredActivos
      .filter((m) => m.tipo === "egreso")
      .forEach((m) => {
        egresosPorCategoria[m.categoria] =
          (egresosPorCategoria[m.categoria] || 0) + m.monto;
      });

    Object.entries(egresosPorCategoria).forEach(([cat, gasto]) => {
      const pct = (gasto / totalIngresos) * 100;
      if (pct >= 35) {
        alertas.push({
          id: `alert-cat-${cat}`,
          mensaje: `💡 Consumo relevante en ${cat}`,
          descripcion: `Has asignado ${currency || getMobileCurrencySymbol()} ${gasto.toFixed(2)} a ${cat} este mes (representa el ${pct.toFixed(0)}% de tus ingresos de este mes).`,
          porcentaje: `${pct.toFixed(0)}% del ingreso`,
        });
      }
    });

    if (totalEgresos > totalIngresos) {
      const deficit = totalEgresos - totalIngresos;
      alertas.push({
        id: "alert-deficit",
        mensaje: "⚠️ Los egresos superan los ingresos del mes",
        color: "#EF4444",
        descripcion: `Llevas un déficit mensual de ${currency || getMobileCurrencySymbol()} ${deficit.toFixed(2)}. Revisa tus metas o disminuye egresos secundarios.`,
        porcentaje: "Déficit",
      });
    }
  }
  ...
  const saldoEsperado =
    ff +
    (cierreResumen?.ingresosTotales || 0) -
    (cierreResumen?.egresosTotales || 0);
  const diferencia =
    saldoFisico !== "" ? Number(saldoFisico) - saldoEsperado : 0;
      ...
      >
          ...
          <View style={{ flex: 1 }}>
            <Text
              style={{
                fontSize: 14,
                fontWeight: "bold",
                color: isDarkMode ? "#F8FAFC" : "#0369A1",
              }}
            >
            ...
            <Text
              style={{
                fontSize: 11,
                color: isDarkMode ? "#94A3B8" : "#0284C7",
                marginTop: 2,
              }}
            >
        ...
        <View style={theme.headerRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={theme.welcomeSubtitle}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={theme.monthSelectorText}>
              📅 {selectedMonth.label} ∨
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {pendientes &&
          pendientes.yesterday &&
          !pendientes.yesterday.isClosed && (
            <View style={theme.arqueoCard}>
              <View style={theme.arqueoHeader}>
                <Text style={theme.arqueoIcon}>📅</Text>
                <View style={{ flex: 1 }}>
                  <Text style={theme.arqueoTitle}>
                    Arqueo Diario: Ayer ({pendientes.yesterday.date})
                  </Text>
                  <Text style={theme.arqueoSummary}>
                    Ingresos:{" "}
                    <Text style={{ color: "#059669", fontWeight: "bold" }}>
                      +${pendientes.yesterday.resumen?.ingresosTotales || 0}
                    </Text>{" "}
                    | Egresos:{" "}
                    <Text style={{ color: "#DC2626", fontWeight: "bold" }}>
                      -${pendientes.yesterday.resumen?.egresosTotales || 0}
                    </Text>
                  </Text>
                </View>
              </View>
              <View style={theme.arqueoActions}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    handleOpenArqueoModal(
                      "diario",
                      pendientes.yesterday.date,
                      pendientes.yesterday.resumen,
                    )
                  }
                  activeOpacity={0.8}
                >
        ...
        {pendientes &&
          pendientes.prevMonth &&
          !pendientes.prevMonth.isClosed && (
            <View
              style={[
                theme.arqueoCard,
            ...
            >
              <View style={theme.arqueoHeader}>
                <Text style={theme.arqueoIcon}>🚨</Text>
                <View style={{ flex: 1 }}>
                  <Text style={[theme.arqueoTitle, { color: "#991B1B" }]}>
                    Cierre Mensual Requerido: {pendientes.prevMonth.periodo}
                  </Text>
                  <Text style={[theme.arqueoSummary, { color: "#B91C1C" }]}>
                    El mes anterior está abierto. Cerrar el mes requiere
                    contraseña y bloqueará sus transacciones.
                  </Text>
                </View>
              </View>
              <View style={theme.arqueoActions}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    handleOpenArqueoModal(
                      "mensual",
                      pendientes.prevMonth.periodo,
                    )
                  }
                  activeOpacity={0.8}
                >
                  <Text style={theme.arqueoBtnText}>
                    🔒 Cerrar Mes Anterior
                  </Text>
                </TouchableOpacity>
              </View>
        ...
        {alertas.map((alerta) => (
          <View
            ...
            style={{
              ...
              borderColor: alerta.color || "#FCA5A5",
              borderLeftWidth: 4,
              borderWidth: 1,
              borderRadius: 12,
              padding: 12,
          ...
          >
            ...
            <Text style={{ color: "#7F1D1D", fontSize: 11, lineHeight: 15 }}>
              {alerta.descripcion}
            </Text>
            <View
              style={{
            ...
            >
              <Text
                style={{ fontSize: 9, color: "#EF4444", fontWeight: "bold" }}
              >
                {alerta.porcentaje}
              </Text>
            </View>
        ...
        <View style={theme.balanceGrid}>
          <View style={[theme.balanceCard, { backgroundColor: "#10B981" }]}>
            ...
            <Text
              style={theme.balanceValue}
              numberOfLines={1}
              adjustsFontSizeToFit
            >
              {currency || getMobileCurrencySymbol()} {totalIngresos.toFixed(2)}
            </Text>
          </View>
          <View style={[theme.balanceCard, { backgroundColor: "#EF4444" }]}>
            ...
            <Text
              ...
              adjustsFontSizeToFit
            >
              {currency || getMobileCurrencySymbol()} {totalEgresos.toFixed(2)}
            </Text>
          </View>
        </View>

        <View style={[theme.balanceCardWide, { backgroundColor: "#3B82F6" }]}>
          <Text style={theme.balanceLabel}>
            Balance Total ({selectedMonth.label})
          </Text>
          <Text
            style={theme.balanceValueWide}
            numberOfLines={1}
            adjustsFontSizeToFit
          >
            {currency || getMobileCurrencySymbol()} {balanceTotal.toFixed(2)}
          </Text>
        </View>
        ...
        <View style={theme.sectionHeaderRow}>
          <Text style={theme.sectionHeader}>
            🏁 Carrera de Compras Planificadas
          </Text>
          <TouchableOpacity
            style={theme.addGoalBtn}
            onPress={() => setShowAddCompraModal(true)}
            activeOpacity={0.8}
          >
        ...
        {comprasConAvance.length === 0 ? (
          <Text style={theme.emptyText}>
            No tienes compras planificadas activas.
          </Text>
        ) : (
          comprasConAvance.map((compra) => (
            <View
              ...
              style={[
                theme.goalCard,
                compra.alcanzado && { borderColor: "#10B981", borderWidth: 2 },
              ]}
            >
              <View style={theme.goalHeader}>
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <Text style={theme.goalTitle}>
                    {compra.item}{" "}
                    {compra.alcanzado ? "🎉 (¡Listo para comprar!)" : ""}
                  </Text>
                  <Text style={theme.goalAmount}>
                    {currency || getMobileCurrencySymbol()}{" "}
                    {compra.montoAhorrado.toFixed(2)} /{" "}
                    {currency || getMobileCurrencySymbol()}{" "}
                    {compra.montoObjetivo.toFixed(2)}
                  </Text>
                </View>
                ...
                <View style={{ flexDirection: "row", gap: 6 }}>
                  ...
                  <TouchableOpacity
                    style={[
                      theme.compradoBtn,
                      compra.alcanzado && { backgroundColor: "#059669" },
                    ]}
              ...
              <View style={theme.goalTrack}>
                <View
                  style={[
                    theme.goalFill,
                    {
                      width: `${compra.porcentaje}%`,
                      backgroundColor: compra.alcanzado ? "#10B981" : "#34D399",
                    },
                  ]}
                />
              </View>
              <Text
                style={[
                  theme.goalPercent,
                  compra.alcanzado && { color: "#10B981", fontWeight: "bold" },
                ]}
              >
                {compra.porcentaje.toFixed(0)}% Completado{" "}
                {compra.alcanzado ? "• ¡Dinero suficiente!" : ""}
              </Text>
            </View>
          ))
        )}
          ...
          <>
            ...
            {recurrentes.map((item) => (
              ...
              >
                ...
                >
                  <View style={{ flex: 1, paddingRight: 10 }}>
                    <Text style={theme.recurrenteTitle} numberOfLines={1}>
                      {item.nombre}
                    </Text>
                    <Text style={theme.recurrenteSubtitle} numberOfLines={1}>
                      {item.categoria} •{" "}
                      {new Date(
                        item.fecha || item.creadoEn,
                      ).toLocaleDateString()}
                    </Text>
                  </View>
                  <Text style={theme.recurrenteAmount} numberOfLines={1}>
                    + {currency || getMobileCurrencySymbol()}{" "}
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    handleCancelarConstancia(item._id, item.nombre)
                  }
                  activeOpacity={0.8}
                >
                  <Text
          ...
          <View style={theme.modalContent}>
            ...
            {MONTH_OPTIONS.map((opt) => (
              <TouchableOpacity
                ...
                style={[
                  theme.monthOption,
                  selectedMonth.label === opt.label && theme.monthOptionActive,
                ]}
                onPress={() => {
          ...
          >
            <View style={theme.modalContent}>
              <Text style={theme.modalTitle}>
                🏛️ Cierre {cierreTipo === "diario" ? "Diario" : "Mensual"}
              </Text>
              <Text
                style={{
                  textAlign: "center",
              ...
              >
                ...
                >
                  ...
                  >
                    +S/ {cierreResumen.ingresosTotales.toFixed(2)}
                  </Text>
                </View>
                ...
                >
                  ...
                  >
                    -S/ {cierreResumen.egresosTotales.toFixed(2)}
                  </Text>
                </View>
                  ...
                  <View style={{ flex: 1, paddingRight: 8 }}>
                    ...
                    >
                      {diferencia === 0
                        ...
                        : diferencia < 0
                          ? "⚠️ Caja no cuadra. Faltante."
                          : "💡 Caja no cuadra. Sobrante."}
                    </Text>
                  </View>
                  <Text
                    style={{
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 18 }}>
                <TouchableOpacity
                  style={theme.cancelBtn}
                  onPress={() => setShowArqueoModal(false)}
                >
                  <Text style={theme.cancelBtnText}>Cancelar</Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  <Text style={theme.saveGoalBtnText}>
                    {submittingCierre ? "Guardando..." : "💾 Guardar Cierre"}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
          ...
          <View style={theme.modalContent}>
            ...
            >
              ...
              <TouchableOpacity onPress={() => setShowTutorialModal(false)}>
                <Text
                  style={{ color: "#9CA3AF", fontSize: 16, fontWeight: "bold" }}
                >
                  ✕
                </Text>
              </TouchableOpacity>
            ...
            {tutorialStep === 0 && (
              <View style={{ alignItems: "center", paddingVertical: 10 }}>
                ...
                >
                  Monitorea en tiempo real tus ingresos, egresos y tu balance
                  total disponible.
                </Text>
              </View>
            )}
            ...
            {tutorialStep === 1 && (
              <View style={{ alignItems: "center", paddingVertical: 10 }}>
                ...
                >
                  Agrega metas de compra. Cada meta evalúa su avance en
                  simultáneo contra tu dinero real sin alterar tus fondos hasta
                  que presiones Comprado.
                </Text>
              </View>
            )}
            ...
            {tutorialStep === 2 && (
              <View style={{ alignItems: "center", paddingVertical: 10 }}>
                ...
                >
                  Realiza cierres diarios y mensuales comparando tu saldo
                  contable con el dinero físico real en caja.
                </Text>
              </View>
            )}
            ...
            {tutorialStep === 3 && (
              <View style={{ alignItems: "center", paddingVertical: 10 }}>
                ...
                >
                  Sube capturas de Yape, Plin o boletas. Gemini Vision
                  autocompletará el monto y concepto en segundos.
                </Text>
              </View>
            )}
              ...
              {[0, 1, 2, 3].map((idx) => (
                <View
                  ...
                  style={{
                    ...
                    backgroundColor:
                      idx === tutorialStep ? "#10B981" : "#6B7280",
                  }}
                />
              ))}
            </View>
