...
// Importar servicio original para fallback - corregido para CommonJS
let originalService;
try {
  originalService = require('../../frontend/src/services/reportesService.js');
} catch (error) {
  console.warn('⚠️ Servicio original reportes no encontrado, usando implementación mínima');
  originalService = {};
}
...
class ReportesAdapter {
  ...
  loadConfig() {
    try {
      ...
      const reportesConfig = adapterConfig.isAdapterEnabled ? adapterConfig.isAdapterEnabled('reportes') : false;
      const sdkConfig = adapterConfig.getSDKConfig ? adapterConfig.getSDKConfig() : { baseURL: 'http://localhost:3000/api' };
      const loggingConfig = adapterConfig.getLoggingConfig ? adapterConfig.getLoggingConfig() : { enabled: false };
      
      return {
        enableSDK: reportesConfig,
        sdkConfig: sdkConfig,
        debug: loggingConfig.enabled
      };
    } catch (error) {
      console.warn('⚠️ Error cargando configuración AdapterConfig, usando defaults');
      return {
        enableSDK: false,
  ...
  async executeWithFallback(sdkMethod, originalMethod, methodName, ...args) {
    ...
    if (!this.isSDKEnabled || this.fallbackToOriginal) {
      ...
      // Verificar que el método original existe
      if (originalMethod && typeof originalMethod === 'function') {
        return await originalMethod(...args);
      } else {
        throw new Error(`Método ${methodName} no disponible en servicio original`);
      }
    }
  ...
  async getReportes(params) {
    // Método original para fallback
    const originalMethod = originalService?.getReportes || (async (params) => {
      // Implementación fallback simple si no existe el original
      console.warn('⚠️ Servicio original no disponible, usando implementación básica');
      return [];
    });
...
// Exportar los métodos manteniendo la misma interfaz (CommonJS)
module.exports = {
  getReportes: (...args) => reportesAdapter.getReportes(...args),
  
  // Métodos nuevos del SDK (opcionales)
  generateReport: (...args) => reportesAdapter.generateReport(...args),
  exportReport: (...args) => reportesAdapter.exportReport(...args),
  getReportStats: (...args) => reportesAdapter.getReportStats(...args),
  
  // Controles del adaptador
  updateToken: (token) => reportesAdapter.updateToken(token),
  getAdapterStats: () => reportesAdapter.getAdapterStats()
};

// También exportaciones individuales para compatibilidad
module.exports.getReportes = (...args) => reportesAdapter.getReportes(...args);
module.exports.generateReport = (...args) => reportesAdapter.generateReport(...args);
module.exports.exportReport = (...args) => reportesAdapter.exportReport(...args);
module.exports.getReportStats = (...args) => reportesAdapter.getReportStats(...args);
module.exports.updateToken = (token) => reportesAdapter.updateToken(token);
module.exports.getAdapterStats = () => reportesAdapter.getAdapterStats();

// Exportar instancia para uso avanzado y debugging
module.exports.reportesAdapter = reportesAdapter;